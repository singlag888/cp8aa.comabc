...
<link href="<?php echo ($webUrl); ?>/Public/Admin/BJUI/themes/css/bootstrap.min.css" rel="stylesheet">
<style type="text/css">
...
function changeCode(){
    $("#captcha_img").attr("src", "<?php echo U('Publics/verfy');?>&t="+genTimestamp());
}
function choose_bg() {
...
function check_username(){
    ...
    if(username != ''){
        $.ajax({
            url: "<?php echo U('Publics/ajax_check_username');?>",
            type: 'POST',
            dataType: 'JSON',
            data:{
                username:username
...
function __login(){
    ...
    $.ajax({
        type: "post",
        url: "<?php echo U('Publics/ajax_login');?>",
        dataType: 'JSON',
        data: {
            "username": username,
        ...
        success: function (data) {
            if (data.flag){
                ...
                location.href = "<?php echo U('AdminCP/index');?>";
            }else{
...
<div class="main_box">
    ...
    <div class="login_box">
        ...
        <div class="login_form">
            <input type="hidden" value="<?php echo ($randomKey); ?>" id="j_randomKey" />
            <form action="/index.php?m=Admin&c=Publics&a=login" id="login_form" method="post">
                ...
                <div class="form-group">
                    <label for="j_password" class="t">密码：</label> <input id="j_password" value="" name="password" type="password" class="form-control x319 in">
                </div>
                <div class="form-group" id="verify">
                    <label for="j_captcha" class="t">验证码：</label> <input id="j_captcha" name="code" type="text" class="form-control x164">
                    <img id="captcha_img" alt="验证码" title="验证码" src="<?php echo U('Publics/verfy');?>" class="m">
                </div>
                <div class="form-group">
                    <label class="t"></label>
